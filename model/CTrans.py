...
class Reconstruct(nn.Module):
    ...
    def forward(self, x,en,shape_end):
        if x is None:
            return None
        ...
        x = nn.Upsample(size=en.shape[2::])(x)
...
class ChannelTransformer(nn.Module):
    def __init__(self, config, vis, img_size, channel_num=[64, 128, 256, 512], patchSize=[32, 16, 8, 4]):
        ...
        self.embeddings_2 = Channel_Embeddings(config,self.patchSize_2, img_size=[img_size[0]//2,img_size[1]//2], in_channels=channel_num[1])
        self.embeddings_3 = Channel_Embeddings(config,self.patchSize_3, img_size=[img_size[0]//4,img_size[1]//4], in_channels=channel_num[2])
        self.embeddings_4 = Channel_Embeddings(config,self.patchSize_4, img_size=[img_size[0]//8,img_size[1]//8], in_channels=channel_num[3])
    ...
    def forward(self,en1,en2,en3,en4):
        ...
        x1 = self.reconstruct_1(encoded1, en1,en4.shape[2::]) if en1 is not None else None
        x2 = self.reconstruct_2(encoded2, en2,en4.shape[2::]) if en2 is not None else None
        x3 = self.reconstruct_3(encoded3, en3,en4.shape[2::]) if en3 is not None else None
        x4 = self.reconstruct_4(encoded4, en4,en4.shape[2::]) if en4 is not None else None
