...
from engine.other import findindex
sep = os.sep
...
def feature_per_volume(feature_list, patient_slice_index):
    ...
    volumes = {}
    num_slices = []
    num_slices_tmp = [p.split(sep)[-1].split('_')[0]+'_'+p.split(sep)[-1].split('_')[1] for p in patient_slice_index]
    for i in num_slices_tmp:
        if 'h5' in i:
            num_slices.append(i.split('_')[0])
        else:
            num_slices.append(i)
    dict_address = findindex(num_slices)
    for p in dict_address.keys():
